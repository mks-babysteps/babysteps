...
function updateBabyPic(req, res, action) {
  return new Q(db.users.findOneAndUpdate({username: req.headers.username, 'children.firstName' : req.body.firstName}, action, {  new: true },
    function(err, user) {
      if(err) {
        console.log(err);
      } else {
        res.send(user);
        console.log("data in update baby pic users/upsertted", user);
      }
  }))
}
    ...
    .then(function(data) {
      ...
      {$addToSet: { 'children': childInfo } }, { new: true }, function(err,user) {
        if(err) {
          ...
          console.log("This is from create",user)
        res.send(user);
      }
      })
    }
